...
- 로그인 실패 / 탈퇴 계정 안내 메시지
- 비밀번호는 **BCrypt 단방향 해싱**으로 저장 (DB에 평문을 두지 않음)
...
- 비밀번호는 **BCrypt 단방향 해싱**으로 저장하고, 로그인 시 `BCrypt.checkpw`로 해시를 비교해 검증합니다. (`PasswordUtil`)
- 클라이언트 검증에만 의존하지 않고 **서버측에서 입력을 다시 검증**합니다. 회원가입 필수값과 게시글/댓글의 빈값을 컨트롤러에서 차단합니다.
...
erDiagram
    ...
    UserInfo ||--o{ Message : "발신"
    UserInfo ||--o{ Message : "수신"
